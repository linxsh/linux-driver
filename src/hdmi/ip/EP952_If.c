...
unsigned char HDMI_Tx_AKSV_RDY(void)
{
	...
	if(status != SMBUS_STATUS_Success) {
		LogFormat(INFO,"ERROR: AKSV RDY - MCU IIC %d\r\n", (int)status);
		return 0;
	}
...
unsigned char HDMI_Tx_read_AKSV(unsigned char *pAKSV)
{
	...
	if(status != SMBUS_STATUS_Success) {
		LogFormat(INFO,"ERROR: AKSV read - MCU IIC %d\r\n", (int)status);
		return 0;
	}
	...
	if(j != 20) {
		LogFormat(INFO,"ERROR: AKSV read - Key Wrong\r\n");
		return 0;
	}
...
unsigned char HDMI_Tx_read_RI(unsigned char *pRI)
{
	...
	if(status != SMBUS_STATUS_Success) {
		LogFormat(INFO,"ERROR: Tx Ri read - MCU IIC %d\r\n", (int)status);
		return 0;
	}
	return 1;
}
...
void HDMI_Tx_AMute_Enable(void)
{
	...
	LogFormat(INFO,"< EP952 Audio_Mute_enable >\r\n");
}
...
void HDMI_Tx_AMute_Disable(void)
{
	...
	LogFormat(INFO,"< EP952 Audio_Mute_disable >\r\n");
}
...
void HDMI_Tx_VMute_Enable(void)
{
	...
	LogFormat(INFO,"< EP952 Video_Mute_enable >\r\n");
}
...
void HDMI_Tx_VMute_Disable(void)
{
	...
	LogFormat(INFO,"< EP952 Video_Mute_disable >\r\n");
}
...
void HDMI_Tx_Video_Config(PVDO_PARAMS Params)
{
	...
	switch(Params->SyncMode) {
		...
		case SYNCMODE_HVDE:
			...
			LogFormat(INFO,"EP952 Set Sync mode to (H+V+DE)input mode\r\n");
			break;
		...
		case SYNCMODE_HV:
			...
			LogFormat(INFO,"EP952 Set Sync mode to (H+V)input + DE_GEN mode\r\n");
			break;
		...
		case SYNCMODE_Embeded:
			...
			if(Params->VideoSettingIndex < EP952_VDO_Settings_Max) {
				...
				LogFormat(INFO,"ERROR: VideoCode overflow E_SYNC table\r\n");
			}

			LogFormat(INFO,"EP952 Set Sync mode to (Embeded)input mode\r\n");
			break;
	}
	...
	switch(Params->FormatIn) {
		...
		case COLORFORMAT_RGB:
			...
			LogFormat(INFO,"EP952 Set to RGB In\r\n");
			break;
		case COLORFORMAT_YCC444:
			...
			LogFormat(INFO,"EP952 Set to YCC444 In\r\n");
			break;
		case COLORFORMAT_YCC422:
			...
			LogFormat(INFO,"EP952 Set to YCC422 In\r\n");
			break;
	}
	switch(Params->FormatOut) {
		...
		case COLORFORMAT_RGB:
			...
			LogFormat(INFO,"EP952 Set to RGB Out\r\n");
			break;
...
void HDMI_Tx_Audio_Config(PADO_PARAMS Params)
{
	...
	if(Params->Interface & 0x08) { // IIS
		LogFormat(INFO,"EP952 set to IIS IN \r\n");
		Temp_Data[0] = 0;
		...
		switch(Params->ADSRate) {
			default:
			case 0: // Bypass
				//LogFormat(INFO,"Audio ADS = 0\r\n");
				FinalADSRate = 0;
				FinalFrequency = Params->InputFrequency;
				break;
			case 1: // 1/2
				//LogFormat(INFO,"Audio ADS = 1_2\r\n");
		...
		LogFormat(INFO,"EP952 set to SPDIF IN \r\n");
		// power up OSC
	...
	if(Params->VideoSettingIndex < EP952_VDO_Settings_Max) {
		...
		if(EP952_VDO_Settings[Params->VideoSettingIndex].HVRes_Type.Vprd % 500) { // 59.94/60 Hz
			...
			//LogFormat(INFO,"EP952 Use N_CTS_Index shift(VFS) = %d\r\n", (int)Params->VFS);
		}
	}
	else {
		LogFormat(INFO,"EP952 Use default N_CTS_Index\r\n");
	...
	LogFormat(INFO,"EP952_CTS_1(Reg addr 0x61) = 0x%02X\r\n",(int)Temp_Data[0]);
	EP952_Reg_Read(EP952_CTS_L, Temp_Data, 1);
	LogFormat(INFO,"EP952_CTS_2(Reg addr 0x62) = 0x%02X\r\n",(int)Temp_Data[0]);
	EP952_Reg_Read(EP952_N_H, Temp_Data, 1);
	LogFormat(INFO,"EP952_N_0(Reg addr 0x63) = 0x%02X\r\n",(int)Temp_Data[0]);
...
SMBUS_STATUS IIC_Write(unsigned char IICAddr, unsigned char ByteAddr, unsigned char *Data, unsigned int Size)
{
	...
	hisi_i2c_write(IICAddr, ByteAddr, Data, Size);
...
SMBUS_STATUS IIC_Read(unsigned char IICAddr, unsigned char ByteAddr, unsigned char *Data, unsigned int Size)
{
	...
	hisi_i2c_read(IICAddr, ByteAddr, Data, Size);
